...
enum ItemCode {
  // 미세먼지
  @HiveField(0) // Type 안에서는 Field가 다른 클래스 Field와 중복이어도 상관없다.
  PM10,
  // 초미세먼지
  @HiveField(1)
  PM25,
...
@HiveType(typeId: 1) // typeId는 겹치지 않아야 한다. Hive에서 같은 타입으로 인식함
class StatModel {
  @HiveField(0) // 등록을 한 번 하면 숫자를 바꾸지 않아야 한다. 만약 지웠으면, 다시 0을 생성하면 안 됨
  final double daegu;
  @HiveField(1)
  ...
  // Hive 타입을 지원한 클래스는 기본 컨스트럭터가 있어야 함
  StatModel({
    required this.daegu,
